...
class Model {
    ...
    fun addSomeNotes() {
        ...
        notes.add(ModelNote(generateID(), "Free China!!!","Chinese people DO NOT agree on CCP's dictatorship.", important = true, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "End CCP!!!", "CCP has killed millions of Chinese by different movements since 1949.", important = true, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "We are sorry for not helping Hongkong in 2019", "I am sorry for what happened in 2019.", important = false, archived = false))
        notes.add(ModelNote(generateID(), "Appreciation", "We appreciate any help along the way", important = false, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "Fight until CCP ends or we die", "people of China", important = false, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "Future", "We will continue to fight.", important = false, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "Resources are limited!", "As CCP is trying to make China go back to an isolated country again, resources will be consumed very fast.", important = false, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "Why?", "Collabrating and doing business with the world make CCP stronger than ever.", important = false, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "It takes time.", "It takes time to overthrow the CCP government because they have grown so much stronger in the past 40 years.", important = false, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "People of China", "We, the people of China want to spread the word that Chinese people did not give up on fighting against CCP.", important = false, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "Continue from above","Although it can still be considered as a \"minor\" protest compared HongKong 2019, it is the " +
                "largest and widest protest and call for freedom since 1989.", important = false, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "1126","In November 26th 2022, protest against Xi and CCP breaks out in many major cities in China.", important = false, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "8964","In June 4th 1989, CCP killed hundreds of young students who peacefully protest for freedom and democracy in Beijing.", important = false, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "BrainWashing","Lots of young Chinese are brainwashed the moment they are born, because they have no way to acquire " +
                "information that is not given by CCP.", important = false, archived = false))
