...
class NotesViewModel : ViewModel() {
    ...
    private val curSelectedNote = MutableLiveData<VMNote>()
    ...
    /**
     * Returns a read-only version of [notes], which stores read-only observables of [VMNote]. Observe to receive
     * notifications about changes to the list.
     * @see notes
     */
    ...
    fun setViewArch() {
        this.viewArchived.value = !this.viewArchived.value!!
    }
    fun getViewArch(): MutableLiveData<Boolean> {
        return this.viewArchived
    }

    fun getCurSelectedNote(): MutableLiveData<VMNote> {
        return this.curSelectedNote
    }

    fun setCurSelectedNote(n: LiveData<VMNote>) {
        curSelectedNote.value = VMNote(n.value!!.id, n.value!!.title, n.value!!.content, n.value!!.important, n.value!!.archived)
    }

    fun updateNoteTextTitle(newNoteID: Int, newTitle: String) {
        model.updateNoteTitle(newNoteID, newTitle)
    }
    fun updateNoteTextContent(newNoteID: Int, newContent: String) {
        model.updateNoteContent(newNoteID, newContent)
    }
    fun updateSelectedNoteLabels(btnType: String, newBool: Boolean) {
        if (btnType == "important") {
            model.updateNoteImportant(curSelectedNote.value!!.id, newBool)
        }
        else if(btnType == "archive") {
            model.updateNoteArchived(curSelectedNote.value!!.id, newBool)
        }
    }

    fun addInArchiNotes() {
        this.notes.value!!.clear()
        var tempList = MutableLiveData<MutableList<MutableLiveData<VMNote>>>(mutableListOf())
        for (n in model.notes) {
            val currNote = VMNote(n.id, n.title, n.content, n.important, n.archived)
            tempList.value!!.add(MutableLiveData(currNote))
        }
        tempList.value?.sortWith { p0, p1 -> model.compareNotes(p0?.value!!.id, p1?.value!!.id) }
        this.notes.value = tempList.value
    }
    fun showNonArchNotes() {
        this.notes.value!!.clear()
        var tempList = MutableLiveData<MutableList<MutableLiveData<VMNote>>>(mutableListOf())
        for (n in model.notes) {
            if (!n.archived) {
    ...
    fun search(searchString: String) {
        var tempList = MutableLiveData<MutableList<MutableLiveData<VMNote>>>(mutableListOf())
        for (n in model.notes) {
            if (n.content.contains(searchString) || n.title.contains(searchString)) {
                val currNote = VMNote(n.id, n.title, n.content, n.important, n.archived)
                tempList.value!!.add(MutableLiveData(currNote))
    ...
    fun deleteNote(id: Int) {
        for (n in this.notes.value!!) {
            if (n.value!!.id == id && !(n.value!!.important)) {
                model.removeNote(id)
                break
            }
        }
    ...
    fun archiveNote(id: Int) {
        for (n in this.notes.value!!) {
            if (n.value!!.id == id) {
                model.updateNoteArchived(id, !(n.value!!.archived))
                break
            }
        }
    }
